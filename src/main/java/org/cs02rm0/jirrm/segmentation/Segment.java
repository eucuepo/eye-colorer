/*
 * Segment.java
 * Created on August 24, 2004, 10:10 PM
 */
...
import org.cs02rm0.jirrm.Utils;
...
public class Segment {
	...
	/*
	 * 0.4 Scaling factor for hough transform Use boundaries to locate top then
	 * bottom eyelid Eliminate eyelids by thresholding - NaN values
	 */
	...
	public void segment(BufferedImage bimg) {
		...
		img = new BufferedImage(bimg.getWidth(), bimg.getHeight(), bimg.getType());
		g = img.getGraphics();
		g.drawImage(bimg, 0, 0, null);
		...
		for (int i = 0; i < circles.size(); i++) {
			final Circle c = (Circle) circles.get("iris");
			Utils.drawCircle(bimg, c.cp, c.radius, Color.BLUE);
		}
		// To Do:
